        ...
        <form action="{{ route('admin.events.update', $event) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <p class="text-[8px] text-slate-400 mt-1 ml-1 uppercase">Para cambiar el lugar, debe crear un evento nuevo.</p>
                </div>
            </div>
            ...
            <div class="bg-slate-50/50 border border-slate-100 rounded-[2.5rem] p-6 md:p-8">
                <div class="flex items-center gap-3 mb-6">
                    ...
                    <h3 class="text-[11px] font-black text-slate-800 uppercase tracking-widest">Ajuste de Precios y Disponibilidad</h3>
                </div>
            ...
            <div class="border-t border-slate-100 pt-6">
                ...
                <div class="flex flex-col md:flex-row gap-6 items-start">
                    <div class="relative w-full md:w-72 h-44 bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200 overflow-hidden group">
                        <img id="image-preview" src="{{ $event->image_path ? asset('storage/' . $event->image_path) : '#' }}" 
                             alt="Vista previa" class="{{ $event->image_path ? '' : 'hidden' }} w-full h-full object-cover">
                    </div>

                    <div class="flex-1">
                        <input type="file" name="image" id="image-input" accept="image/*"
                               class="block w-full text-sm text-slate-500 file:mr-4 file:py-3 file:px-6 file:rounded-full file:border-0 file:text-xs file:font-black file:uppercase file:bg-slate-100 file:text-slate-700 hover:file:bg-slate-200 cursor-pointer">
                        <p class="text-[9px] text-slate-400 mt-3 font-bold uppercase tracking-tight italic">Deje en blanco para mantener la imagen actual.</p>
                    </div>
                </div>
            </div>
