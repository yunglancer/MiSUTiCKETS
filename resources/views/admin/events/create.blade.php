        ...
        <form action="{{ route('admin.events.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    @error('venue_id') <p class="text-rose-500 text-[9px] font-bold mt-1 ml-1 uppercase">{{ $message }}</p> @enderror
                </div>
            </div>
            ...
            <div class="border-t border-slate-100 pt-6">
                ...
                @error('image') <p class="text-rose-500 text-[9px] font-bold mt-1 ml-1 uppercase">{{ $message }}</p> @enderror
            </div>

            {{-- Destacado --}}
            <div class="flex items-center ml-1 bg-slate-50 p-4 rounded-2xl border border-slate-100 w-max">
                <input type="checkbox" name="is_featured" id="is_featured" {{ old('is_featured') ? 'checked' : '' }} class="h-4 w-4 text-[#FF6600] border-gray-300 rounded focus:ring-[#FF6600]">
