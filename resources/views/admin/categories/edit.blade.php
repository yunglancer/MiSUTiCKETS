    ...
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            <div class="flex items-center gap-3">
                <h1 class="text-2xl font-black text-slate-900 uppercase tracking-tight">Editar Categoría</h1>
                <span class="bg-slate-100 text-slate-500 text-[10px] font-black px-3 py-1 rounded-lg uppercase tracking-widest">ID: #{{ $category->id }}</span>
            </div>
            <p class="text-slate-500 text-[10px] uppercase tracking-widest font-bold mt-1">Modifica el nombre o el icono visual</p>
        </div>
    </div>

    {{-- Formulario de Edición --}}
    <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" class="bg-white rounded-3xl border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden">
        @csrf
        @method('PUT')
                    ...
                    <div class="grid grid-cols-3 sm:grid-cols-5 md:grid-cols-8 gap-3">
                        <template x-for="icon in icons" :key="icon.name">
                            <button type="button" 
                                ...
                                :class="selectedIcon === icon.name ? 'border-[#FF6600] bg-[#FF6600]/5 text-[#FF6600] shadow-md' : 'border-slate-50 bg-slate-50/50 text-slate-400 hover:border-slate-200'"
                                class="relative flex flex-col items-center justify-center p-4 rounded-2xl border-2 transition-all duration-200 group">
